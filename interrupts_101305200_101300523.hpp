...
struct memory_partition_t {
    ...
    memory_partition_t(unsigned int p, unsigned int s, std::string c)
        : partition_number(p), size(s), code(c) {}
};
...
struct PCB {
    ...
    PCB(unsigned int pid, int ppid, std::string name, unsigned int sz, int part)
        : PID(pid), PPID(ppid), program_name(name), size(sz), partition_number(part) {}
};
...
bool allocate_memory(PCB* p) {
    for (int i = 5; i >= 0; i--) {
        if (memory[i].size >= p->size && memory[i].code == "free") {
            p->partition_number = memory[i].partition_number;
            memory[i].code = p->program_name;
            return true;
        }
    }
    return false;
...
void free_memory(PCB* p) {
    if (p->partition_number >= 1 && p->partition_number <= 6) {
        memory[p->partition_number - 1].code = "free";
    }
