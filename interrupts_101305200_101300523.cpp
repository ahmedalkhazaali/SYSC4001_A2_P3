...
#include "interrupts_101305200_101300523.hpp"
...
std::tuple<std::string, std::string, int> simulate_trace(std::vector<std::string> trace_file, int time, std::vector<std::string> vectors, std::vector<int> delays, std::vector<external_file> external_files, PCB current, std::vector<PCB> wait_queue) {
    ...
    for(size_t i = 0; i < trace_file.size(); i++) {
        ...
        if(activity == "CPU") { //As per Assignment 1
            ...
            PCB parentProc = current;

            unsigned int freshPid = pidCounter;
            pidCounter = pidCounter + 1;

            //chil starts without a partition, same program+size
            PCB childProc(freshPid, parentProc.PID, parentProc.program_name, parentProc.size, -1);

            // give child its own memory slot 
            allocate_memory(&childProc);

            wait_queue.push_back(parentProc);
            current = childProc;

            execution += std::to_string(current_time) + ", " + std::to_string(duration_intr)
                + ", copy PCB to child process\n";
            current_time += duration_intr;

            execution += std::to_string(current_time) + ", 0, run dispatcher\n";
            execution += std::to_string(current_time) + ", 1, finish fork handler\n";
            current_time += 1;

            system_status += "time: " + std::to_string(current_time) + "; event: " + trace + "\n";
            ...
            auto forkChildRun =simulate_trace(child_trace,
                current_time,
                vectors,
                delays,
                external_files,
            ...
            execution+=std::get<0>(forkChildRun);
            system_status += std::get<1>(forkChildRun);
            current_time=std::get<2>(forkChildRun);
            if (!wait_queue.empty()) {
                wait_queue.pop_back();
            }
            current =parentProc;
            ...
            int fileSize = (int)get_size(program_name, external_files);

            execution += std::to_string(current_time) + ", "
                + std::to_string(duration_intr)
                + ", target program size " + std::to_string(fileSize) + " Mb\n";
            current_time += duration_intr;
            ...
            if (fileSize > 0) {
                current.size = (unsigned int)fileSize;
            }

            if (!allocate_memory(&current)) {
                std::cerr << "unable to load program into any region\n";
            }

            int loadTime = fileSize * 15;
            execution += std::to_string(current_time) + ", "
                + std::to_string(loadTime)
                + ", move program into memory partition\n";
            current_time += loadTime;

            execution += std::to_string(current_time) + ", 3, flag slot as used\n";
            current_time += 3;

            execution += std::to_string(current_time) + ", 6, update PCB fields\n";
            current_time += 6;

            execution += std::to_string(current_time) + ", 0, choose next runnable\n";
            execution += std::to_string(current_time) + ", 1, exit exec handler\n";
            current_time += 1;
            ...
            auto execRun = simulate_trace(exec_traces,
                current_time,
                vectors,
                delays,
                external_files,
            ...
            execution += std::get<0>(execRun);
            system_status += std::get<1>(execRun);
            current_time = std::get<2>(execRun);
            break;
        }
    }
